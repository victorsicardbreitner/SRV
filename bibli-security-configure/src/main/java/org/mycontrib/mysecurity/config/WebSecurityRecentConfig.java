...
public class WebSecurityRecentConfig {
	...
	public AuthenticationManager authenticationManagerFromHttpSecurity(HttpSecurity http) throws Exception {
		...
		logger.info("####### authenticationManagerBuilder : "+authenticationManagerBuilder.toString());
		...
		if (myExclusiveUserDetailsService != null) {
			...
			if (mySecurityProperties != null && mySecurityProperties.getJdbcRealm() != null) {
				...
				DataSourceProperties dsProps = mySecurityProperties.getJdbcRealm(); // Ce qu'on met dans le fichier withSecurity.properties
				logger.info(
			...
			if (myAdditionalUserDetailsService != null) {
				logger.info("####### myAdditionalUserDetailsService");
	...
	protected SecurityFilterChain filterChain(HttpSecurity http, AuthenticationManager authenticationManager)
		...
		http.authorizeRequests()
				...
				.antMatchers(staticWhitelist).permitAll()
				.antMatchers(SWAGGER_AUTH_WHITELIST).permitAll()
				.antMatchers(HttpMethod.POST, "/api-login/public/login").permitAll()
				.antMatchers(apiWhitelist).permitAll()
				//.antMatchers(HttpMethod.POST,apiReadonlyWhitelist).denyAll() // tentative 
				.antMatchers(HttpMethod.GET, apiReadonlyWhitelist).permitAll()
				.antMatchers(apiReadonlyWhitelist).hasRole("ADMIN") // toutes les autres requêtes sur les apiReadonlyWhitelist necessitent un rôle ADMIN
	            ...
	            */
	            ...
	            //interdire (sauf authentification) toutes les autres requêtes
