...
public class MyAppGlobalUserDetailsConfigHelper  {
	...
	private static Logger logger = LoggerFactory.getLogger(MyAppGlobalUserDetailsConfigHelper.class);
    ...
    public UserDetailsManagerConfigurer initJdbcGlobalUserDetails(final AuthenticationManagerBuilder auth, MySecuritySimpleConfigurer mySecuritySimpleConfigurer,DataSourceProperties dsProps) throws Exception {
		
    	initRealmDataSource(dsProps);
    	
		JdbcUserDetailsManagerConfigurer jdbcUserDetailsManagerConfigurer =  auth.jdbcAuthentication().dataSource(realmDataSource);

		
		if(isRealmSchemaInitialized()) {	
			logger.info("####### isRealmSchemaInitialized");
    ...
    public UserDetailsManagerConfigurer initInMemoryGlobalUserDetails(final AuthenticationManagerBuilder auth, MySecuritySimpleConfigurer mySecuritySimpleConfigurer)throws Exception {
    	
    	logger.info("####### initInMemoryGlobalUserDetails");
